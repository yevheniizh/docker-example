...
const form =
  '<div><form action="/add"><input name=text><input type="submit"></form></div>';

const index = async (req, res) => {
  const result = await client.query('select * from messages');
  const messages =
    result.rows && result.rows.map((row) => `<div>${row.text}</div>`).join('');
...
const add = async (req, res) => {
  ...
  const result = await client.query('insert into messages values ($1)', [
    parts.query.text,
  ]);
  redirect(res, 302, '/');
};
